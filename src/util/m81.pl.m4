...
m4_define(<++CHECK_ENV++>,
<++@$1 = split( /,|:|\s+/, $ENV{ $1 });
debugprint( "Grabbing $1 from env: $ENV{ $1 } - $$1" );
unless (scalar @$1) {
    my $regexp = _derive_lookahead_regexp( '^PLACEHOLDER$', '$1', %M80ALIASES ) ;
    push @$1, map { split( /,|:|\s+/, $ENV{$_}) } grep { /$regexp/ } keys %ENV;
    debugprint( "derived $1 from environment - @$1" );

}
#end("specify $1") if scalar keys %M80ALIASES && !scalar @$1;
debugprint "$1 = @$1\n"++>);

##m4_define([EXIT_ENV],[end( "specify \$1" ) unless ]);
m4_divert m4_dnl ;
package main;
...
CHECK_ENV(M80PATH);
unshift @M80PATH, ('M80_LIB/perl', '.'); #default paths
...
if ($recurse) {
    ...
    require "M80_BIN/perl_recurse_function.pl";
} else {
    for my $fn (@functions) {  
...
sub sourceexec {
    ...
    debugprint( "cd $dir && SHELL --noprofile --norc -c '. $file && env' - suppress_stdout=$suppress_stdout" );
    _print( "cd $dir && SHELL --noprofile --norc -c '. $file && env' | perl -ple 's/^(.+?)=(.+)\$/export \$1=\$2/'", $suppress_stdout );
...
sub genTemplateAPI {
    my $template = "M80_LIB/../templates/genTemplateAPI.tmpl";
...
This document describes VERSION of m81
...
cb(-z, increase debugging verbosity levels)
cb(--load || --libs, the files that should be execed in order. These
    are going to be investigated to determine how to convert them)
cb(--path, the paths to search for files in order)
cb(--suppress-stdout, if possible - try not to print things out - just alter the internal env)
cb(--recurse, will load the perl_recurse_function script which will apply a function to
    all files and subdirs in a directory.)
cb(--args || -o, an array of function arguments that should be passed to a function invoked
    on the command line. Supports simple arguments only (scalars).)
cb(--exec, a string that should be perl evaled. This overrides the default behaviour or
    passing to the m80 script.)
ceb(--aliases, name=value pairs of aliases for the M80PATH and M80LOAD
...
sb(.m80, make the file (with the .m80 extension removed) and exec it)
cb(.pl, try to perl require it and exec it. If the exec is successful, alter the env based on
on the returned export statements - but do not print that info to stdout (it gets printed
during the require))
cb(.pm, require it)
ceb(.m4, assume that it is an old m80 style repository and m80 --export it)
...
sb(M80PATH, type - list. Specify all paths that the m81 tool should look for libraries)
cb(M80LOAD, type - list. Specify all libraries/scripts that the m81 tool should exec)
ceb(M80ALIASES, type - hash. Specify alias(es) for the M80PATH or M80LOAD env
...
onto PERL5LIB. The default M80PATH is (M80_LIB/perl, .).
...
sb(sourceexec, C<cd $dir && SHELL --noprofile --norc -c 'source $file && env'>)
cb(m80repo, C<DEBUG=$DEBUG QUIET=true M80_REPOSITORY=$dir M80_BDF=$file m80 --export>)
cb(requireonly, C<{ no warnings; require "$file"; }>)
cb(requireexec, C<{ no warnings; require "$file"; }; system( "(cd $dir && ./$file)");>
There is a little more than this going on, but this is the jist. STDOUT from the 
second command is suppressed.)
ceb(makeexec, C<(export DEBUG=$DEBUG && cd $dir && make $file && ./$file)>)
...
sb(dump_env, print out the keys of the \%ENV hash)
cb(eval, --e flag -  it will join all --args together and exec the string.)
cb(cat, given a file, it will cat it to STDOUT)
cb(genTemplateAPI, create the file representing the template API and return a 'source'
    statement that can be evaled)
cb(generate, expand a file with the m80path library)
cb(dump_namespace, print the functions and hashes that exist in a namespace)
cb(dump_functions, print the functions in the global namespace)
cb(dump_hashes, print the hashes in the global namespace)
ceb(dump_arrays, print the arrays in the global namespace)
