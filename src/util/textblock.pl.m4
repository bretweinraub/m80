...
This document describes VERSION 0.0.x of textblock
...
cb(--start, specify a start tag. Default '<:')
cb(--end, specify a end tag. Default ':>')
cb(--no-comments, strip leading comments. Assumes line comments.)
cb(--comment-char, specify the comment char. Default '#')
cb(--full-text, make an attempt to return the text around the block as well as text in the block)
cb(--use-context, specify a library to perform expansion on the "blocked" text)
ceb(--preserve-block, don't truncate the block delimiters)
