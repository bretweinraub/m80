...
# $1 : the $data{} name, $2: the env var suffix
m4_define([APPEND_VAR],
[
    for my $tmp ( @{ $data{ '$1' } } ) {
        $o .= "export $data{name}_$2=\"\${$data{name}_$2} $tmp\"\n";
    }
])

m4_define([APPEND_VAR_LOOP], [m4_foreach(XYZ, ($*), [_cat([APPEND_VAR], XYZ)])])

m4_define([EXTERNAL_COMMAND], 
[    print "running $1\n";
    system( "$1");])
...
sub add2role {
    ...
    APPEND_VAR(nodes, NODES)
    APPEND_VAR_LOOP((data_include, DATA_INCLUDE),
                    (bin_include, BIN_INCLUDE),
...
sub getarray {
    ...
    if (scalar @data > 1) {
        ...
        if ( ref( $data ) eq 'CODE' ) {
            ...
            debugprint( "getarray: \@data has regular data: $data[0]" );
            return $data;
        }
    }
}
...
sub makeMakefileam {
    ...
    templator_call( $dir, 'M80_LIB/../templates/makefileam.tmpl', './Makefile.am');
...
sub makeTopLevelFiles {
    ...
    templator_call($dir, 'M80_LIB/../templates/autoconfTemplate', '.');
...
#    $p->generate;
    ...
    EXTERNAL_COMMAND(aclocal)
    EXTERNAL_COMMAND(autoconf)
    EXTERNAL_COMMAND(automake -ac)
