...
sub writeTime
{
    ...
    my $/usr/local/m80-0.07 = $arg->{/usr/local/m80-0.07};
    my $value = $arg->{value};
    my $extraSelectFlags = $arg->{extraSelectFlags};

    print $indent . "<select name=\"$/usr/local/m80-0.07"."_time\" $extraSelectFlags>\n";
    ...
    print $indent . "</select>\n";
    print $indent . "<select name=\"$/usr/local/m80-0.07"."_ampm\" $extraSelectFlags>\n";
    $_ = $value->{ampm};
    if ($value && /PM/) {
